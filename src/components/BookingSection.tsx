...
export default function BookingSection({
      ...
      if (displayElement) {
        ...
        if (h5Element) {
          ...
          h5Element.textContent =
            formatted || (inputId === "start" ? "Arrive" : "Depart");
        }
      }
    };
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isMobileView) {
        ...
        const isClickInsideGuestPicker =
        ...
        const isClickInsideCalendar = (event.target as Element)?.closest(
          ".flatpickr-calendar",
        );
        ...
        const target = event.target as Node;
        const isClickInsideUnifiedPopup =
          unifiedPopupRef.current?.contains(target);
        const isClickInsideGuestPicker =
          guestPickerRef.current?.contains(target);
        const isClickInsideDateInputs =
          startWrapperRef.current?.contains(target) ||
  ...
  const guestDisplayText =
    childCount > 0 ? `${adultText} / ${childText}` : adultText;
