...
export default function CalendarPage({
  ...
  let activities: Activity[] = Array.isArray(activitiesProp)
    ? activitiesProp
    : [];
  ...
  useEffect(() => {
    ...
    const updatePagination = (index?: number) => {
      ...
      for (let page = 0; page < totalPagesCount; page++) {
        const boundary =
          page === totalPagesCount - 1 ? lastStartIndex : page * perPage;
  ...
  const renderActivityContent = (activity: Activity) => (
    <>
      ...
      <div className="activity-content">
        {activity.title && <h5 className="activity-title">{activity.title}</h5>}
        ...
        {activity.description && (
          <div className="activity-description">
            {Array.isArray(activity.description) ? (
              ...
              <div>
                {truncateToSentences(
                  stripHtmlToPlainText(activity.description),
                )}
              </div>
            )}
          </div>
          ...
          <div className="activity-date-time">
            ...
            {(activity.startsAt || activity.timeRange?.startTime) && (
              <>
                ...
                {activity.startsAt
                  ? activity.endsAt
                    ? `${formatTimeFromIso(activity.startsAt)} – ${formatTimeFromIso(activity.endsAt)}`
                    : formatTimeFromIso(activity.startsAt)
  ...
  return (
    <section
      id={id}
      className={`calendar-section layout-${layout}${disableCarousel ? " no-carousel" : ""} h-pad`}
    >
      {heading && (
        <AnimateIn className="calendar-section-heading">
          <h4>{heading}</h4>
        </AnimateIn>
      )}
      ...
      <div className={`row-lg calendar-grid layout-${layout}`}>
        {activities.map((activity) => (
          <AnimateIn
            key={activity._id}
            className={`calendar-item ${getGridColumnClass()}`}
          >
          ...
          >
            {activities.map((activity) => (
              <SplideSlide key={`carousel-${activity._id}`}>
                <AnimateIn className="calendar-item">
                  {renderActivityContent(activity)}
                </AnimateIn>
              </SplideSlide>
            ))}
